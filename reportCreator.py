...
def convertDictToArray(table, header, fontSize):
    ...
    for key, value in table.items():
        if key in ("0", "1", "2", "3", "4"):
            header.append(
                [Paragraph(str(val), wordStyle) for val in list(value.values())]
            )
        elif not isinstance(table[key], dict):
            header.append(
                [Paragraph(key, wordStyle)]
                + [Paragraph(str(round(value, 2)), wordStyle)]
            )
        else:
            header.append(
                [Paragraph(key, wordStyleBold)]
                + [Paragraph(str(val), wordStyle) for val in list(table[key].values())]
            )
...
def addSquaresInTable(
    table,
    colWidthTableCell,
    rowHeightTableCell,
    firstrenameDictionary,
    firstColorMap,
    secondRenameDictionary=None,
    secondColorMap=None,
    stilColorMap=None,
):
                            ...
                            [
                                [
                                    SquareFlowable(
                                        ...
                                        value=value,
                                        width=colWidthTableCell,
                                        height=rowHeightTableCell,
                                    )
                                ]
                            ]
                        )
...
def page1TableContainer(image, table, height):
    ...
    restructuredTable = addSquaresInTable(
        table,
        colWidth80,
        rowHeightTableCell,
        segmentRename,
    ...
    tableAndImage = [
        [Image(image, width=150, height=100, kind="proportional"), newTable]
    ]
...
def createPdf(folder_path):
    ...
    # Page 5 info
    segment_overlay_nuclei_heatmap = os.path.join(
        folder_path, "segment_overlay_nuclei_heatmap.png"
    )
    ...
    for outer_key, inner_dict in histologicalScoreData.items():
        histologicalScoreData[outer_key] = {
            key: value for key, value in inner_dict.items() if key != ""
        }

    histologicalScoreTable = convertDictToArray(
        histologicalScoreData,
        [["", "AI score", "AI translated score", "Pathologist score"]],
        6,
    )
    segmentAreaTable = []
    ...
    cellTypeCountTable = []
    cellTypeHeader = [
        Paragraph(i, wordStyleBold)
        for i in ["Cell type", "Total count", "per mm²", "per 1000 tumor cells"]
    ]
    ...
    for key, values in cellTypeCountData.items():
        ...
        per_1000_epithelial_cells = Paragraph(
            str(values["per 1000 epithelial cells"]), wordStyle
        )
        # Create a list with the extracted values
    ...
    cellTypePercentTable[0] = [
        segmentRename.get(header, header)
        for header in cellTypePercentTable[0]
        if header != " Cell type"
    ]
    ...
    for key in cellTypeCountData.keys():
        if (
            cellTypeCountData[key]["Celltype"] != "MIT"
            and cellTypeCountData[key]["Celltype"] != "mimi"
        ):
    ...
    histologicalScoreTable[4][3] = Table(
        [[InteractiveTextField(name="Overall", x=5, y=-8)]]
    )
    ...
    cellTypeCountTableContainer = page1TableContainer(
        nuclie_heatmap, cellTypeCountTable, 18
    )
    cellTypePercentTableContainer = page1TableContainer(
        segment_overlay_nuclei_heatmap, cellTypePercentTable, 25
    )

    majorContainer = Table(
        [
            [histologyTableContainer],
            [segmentAreaTableContainer],
            [cellTypeCountTableContainer],
            [cellTypePercentTableContainer],
        ]
    )
    ...
    for key, value in cellTypeCountData.items():
        for nestedKey, nestedValue in cellTypeCountData[key].items():
            if cellTypeCountData[key][nestedKey] == "MIT":
                mitoticInfoTable.append(
                    ["Total count", str(cellTypeCountData[key]["Total count"])]
                )
                mitoticInfoTable.append(
                    ["per mm²", str(round(cellTypeCountData[key]["per mm2"], 2))]
                )
                mitoticInfoTable.append(
                    [
                        "per 1000 tumor cells",
                        str(cellTypeCountData[key]["per 1000 epithelial cells"]),
                    ]
                )
    ...
    mitoticTableContainer = Table(
        [
            [
                (
                    Image(hpfThumbnail, width=150, height=100, kind="proportional")
                    if hpfThumbnail != ""
                    else ""
                ),
                mitoticTable,
    ...
    mitoticTableContainer.setStyle(
        TableStyle(
            [
                ("ALIGN", (0, 0), (-1, -1), "CENTER"),
                ("VALIGN", (0, 0), (-1, -1), "TOP"),
                    ...
                    for searchFile in allFiles:
                        if f"HPF_{hpfNumber[0]}.png" in searchFile:
                            ...
                            if hpfNumber[0] in hpfLocationDictionary:
                                hpfLocationDictionary[hpfNumber[0]][file_path].append(
                                    subHpf_file_path
                                )
    ...
    for hpfNumber, mainHpfPath in sortedHpfDictionary.items():
        for mainHpf, subHpfList in mainHpfPath.items():
            images = [
                Image(image_path, width=15, height=15, kind="proportional")
                for image_path in subHpfList
            ]
            ...
            subHpfTableContainer = Table(
                subHpfTable, colWidths=[20] * 5, rowHeights=[20] * 5
            )
            ...
            firstHpf = Table(
                [
                    [
                        Image(mainHpf, width=150, height=100, kind="proportional"),
                        subHpfTableContainer,
                    ]
                ]
            )
    ...
    for row in range(num_rows):
        ...
        for column in range(num_columns):
            ...
            )  # Calculate the index in the 'allHpf' list
            if index < max_cells:
    ...
    cleanedVescularDensity = convertDictToArray(
        vescularityDensity, [["Statistics", "Value"]], 7
    )
    cleanednucleoliData = convertDictToArray(
        nucleoliData, [["Confidence", "Proportion of cells with prominent nucleoli"]], 7
    )
    ...
    nuclearTableStyle = TableStyle(
        [
            ...
            (
                ...
                (-1, -1),
                "Times-Roman",
            ),  # First row, excluding the first cell
            ("FONTSIZE", (0, 0), (-1, -1), 8),
            ("VALIGN", (0, 0), (-1, -1), "TOP"),
            ("GRID", (0, 0), (-1, -1), 1, colors.black),
        ]
    ...
    nuclearAIScore = str(
        histologicalScoreData["Nuclear pleomorphism"]["AI translated score"]
    )
    nucleiScoreText = f"AI derived Nuclear pleomorphism score is {nuclearAIScore}"

    nucleiImageText = (
        "*In the image above, the yellow boxes represent prominent nucleoli"
    )

    nucleiImageTextStyles = ParagraphStyle(
        name="Normal", alignment=enums.TA_CENTER, fontName="Times-Italic", fontSize=8
    )
    ...
    NuclearPageTables = [
        [
            Image(sideDensityImage, width=180, height=200, kind="proportional"),
    ...
    NuclearPageTablesContainer.setStyle(
        TableStyle([("VALIGN", (0, 1), (-1, 1), "TOP")])
    )
    ...
    if hpfNucleoliImage != "":
        NucleiImageContainer = Table(
            [
                [Image(hpfNucleoliImage, width=350, height=350, kind="proportional")],
                [Paragraph(nucleiImageText, nucleiImageTextStyles)],
            ]
        )
        rowh = len(legendData)
        rowHeights = [15] * rowh
        ...
        NucleiImageLegendContainer.setStyle(
            TableStyle(
                [
                    ("VALIGN", (0, 0), (-1, -1), "TOP"),
                ]
    ...
    data = [
        ...
        [
            Paragraph("Total no. in CT", wordStyle),
            Paragraph(str(tbInfo["total_number_tubules_in_ct_10"]), wordStyle),
        ],
        [
            Paragraph("Total percentage in CT", wordStyle),
            Paragraph(
                str(round(tbInfo["total_percentage_tubule_area_in_ct"], 2)), wordStyle
            ),
        ],
        [
            Paragraph("per mm²", wordStyle),
            Paragraph(str(round(tbInfo["Tubule/mm2"], 2)), wordStyle),
        ],
        [
            Paragraph("AI derived Tubule/Acinar formation score", wordStyleBold),
            Paragraph(str(tbInfo["Tubule score"]), wordStyleBold),
        ],
    ]

    tubuleTable = Table(data, colWidths=[100, 50])
    tubuleTable.setStyle(
        TableStyle(
            [
                ("ALIGN", (0, 0), (-1, -1), "CENTER"),
                (
    ...
    stilCellTableHeader = ["no. of cells per mm²"] + list(
        stilCellTable[next(iter(stilCellTable))].keys()
    )
    ...
    restructuredStilCellTypeTable = addSquaresInTable(
        cleanedStilCellTable,
        colWidth80,
        [18],
        cellTypeRename,
        nucleiColorMap,
        stilColorMap=stilColorMap,
        secondRenameDictionary=None,
        secondColorMap=None,
    )
    stilCellTypeArrayTable = Table(
        restructuredStilCellTypeTable, colWidths=colWidth, rowHeights=[25, 25, 25, 25]
    )
    ...
    stilTableContainer = [
        [
            Image(
                segment_overlay_nuclei_heatmap,
                width=200,
                height=100,
                kind="proportional",
            )
    ...
    stilTableContainerTable = Table(
        stilTableContainer, rowHeights=[0.6 * 200, 0.4 * 200]
    )
    stilTableContainerTable.setStyle(stilTableContainerStyle)

    wholeBigContainer = [
        [
            stilTableContainerTable,
            Image(thumbnail, width=200, height=200, kind="proportional"),
        ],
    ]

    containerTable = Table(
        wholeBigContainer, colWidths=[570 / 2, 570 / 2], rowHeights=200
    )
    ...
    def add_header(canvas, doc):
        ...
        patientDetails = [
            [
                [
                    ...
                    InteractiveTextField(ID, "ID", 10),
                ],
                [
                    Paragraph("<b>Age:</b>", label_style),
                    InteractiveTextField(Age, "Age", 14),
                ],
                [
                    Paragraph("<b>Gender:</b>", label_style),
                    InteractiveTextField(Gender, "Gender", 25),
                ],
            ],
            [
                [
                    Paragraph("<b>ER:</b>", label_style),
                    InteractiveTextField(ER, "ER", 12),
                ],
                [
                    Paragraph("<b>PR:</b>", label_style),
                    InteractiveTextField(PR, "PR", 12),
                ],
                [
                    Paragraph("<b>Her2:</b>", label_style),
                    InteractiveTextField(Her2, "HER2", 17),
                ],
                [
                    Paragraph("<b>Stage:</b>", label_style),
                    InteractiveTextField(Stage, "Stage", 19),
                ],
                [
                    Paragraph("<b>PAM50:</b>", label_style),
                    InteractiveTextField(PAM50, "PAM50", 25),
                ],
            ],
        ]
        pageTitles = [
        ...
        table.setStyle(
            TableStyle(
                [("ALIGN", (0, 0), (-1, -1), "LEFT"), ("FONTSIZE", (0, 0), (-1, -1), 8)]
            )
        )
    ...
    def add_footer(canvas, doc):
        ...
        styles = ParagraphStyle(name="Normal", fontName="Times-Roman", fontSize=8)
        ...
        text = (
            f'<link href="{link}"><u><font color="blue">{link_text}</font></u></link>'
        )
    ...
    template = PageTemplate(
        id="my_template", frames=[frame], onPage=add_header, onPageEnd=add_footer
    )
